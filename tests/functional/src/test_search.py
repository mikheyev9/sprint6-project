...
@pytest.mark.parametrize("query_data, expected_answer, index, endpoint", SEARCH_PARAM)
@pytest.mark.asyncio
async def test_films_search(make_get_request, redis_clean, query_data, expected_answer, index, endpoint):
    """
    ...
    """
    await redis_clean()
    status, body, timestamp = await make_get_request(endpoint, query_data)
    assert status == expected_answer["status"], (
        f"Проверьте, что get-запрос к `/api/v1/{endpoint}` " f"возвращает ответ с кодом {expected_answer['status']}."
    )
    ...
    if expected_answer["status"] == 200:
        status_redis, body_redis, timestamp_redis = await make_get_request(endpoint, query_data)
