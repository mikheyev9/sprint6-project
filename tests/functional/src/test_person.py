...
async def test_person_not_found(make_get_request, endpoint):
    ...
    assert status == HTTPStatus.NOT_FOUND, (
        f"Проверьте, что get-запрос к `/api/v1/{endpoint}` " f"возвращает ответ с кодом 404."
    )
    assert "detail" in body.keys(), f"Проверьте, что get-запрос к `/api/v1/{endpoint}` " f"возвращает описание ошибки."
...
async def test_person(make_get_request, redis_clean):
    ...
    assert status == HTTPStatus.OK, (
        f"Проверьте, что get-запрос к `/api/v1/{endpoint}` " f"возвращает ответ с кодом 200."
    )
    assert body.get("full_name") == PERSONS[0]["full_name"], (
        f"Проверьте, что get-запрос к `/api/v1/{endpoint}` " f"возвращает правильные данные."
    )
    assert "films" in body.keys(), (
        f"Проверьте, что get-запрос к `/api/v1/{endpoint}` " f"возвращает ответ с данными о фильмах."
    )
    assert len(body.get("films")) == len(PERSONS[0]["films"]), (
        f"Проверьте, что get-запрос к `/api/v1/{endpoint}` " f"возвращает правильное количество фильмов."
    )
    status_redis, body_redis, timestamp_redis = await make_get_request(endpoint)
    assert timestamp > timestamp_redis, (
        f"Проверьте, что get-запрос к `/api/v1/{endpoint}` " f"возвращает ответ из кеш Redis."
    )
    assert status_redis == HTTPStatus.OK, (
        f"Проверьте, что get-запрос в Redis к `/api/v1/{endpoint}` " f"возвращает ответ с кодом 200."
    )
...
async def test_person_film(make_get_request, redis_clean):
    ...
    assert isinstance(body, list), f"Проверьте, что get-запрос к `/api/v1/{endpoint}` " f"возвращает список."
