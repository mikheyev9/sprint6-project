...
from functional.testdata.test_param.film_param import AbstractDTO, MoviesList  # noqa
...
def load_data_from_json(index: str) -> AbstractDTO:
    ...
    if model_class:
        try:
            ...
            logger.error(f"Ошибка при создании экземпляра {model_class.__name__}: " f"{str(e)}")
            return None
