...
class BaseService(Generic[T], ABC):
    ...
    def __init_subclass__(cls, **kwargs):
        ...
        if not hasattr(cls, "service_name") or not cls.service_name:
            raise TypeError(f"Class {cls.__name__} must define a `service_name` attribute.")

        BaseService._registry[cls.service_name] = cls
    
    
    def __init__(self, elastic: AsyncElasticsearch, index: str, model: Type[T]):
    ...
    async def search(self, **kwargs) -> List[T]:
        """
        Поиск объектов в Elasticsearch. Конкретные сервисы должны реализовать этот метод.
        """
    ...
    @classmethod
    @lru_cache()
    def get_instance(cls, service_type: str, elastic: AsyncElasticsearch) -> "BaseService":
